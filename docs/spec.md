...
- Multi-cluster Kubernetes cockpit with kubeconfig context switching, scoped resource exploration, pinned resources, sortable signal-aware inventory with inline pod diagnostics, pod-to-node and node-to-pod debugging jumps, pod-level status diagnostics, pod condition diagnostics, newest-first warning-prioritized pod events, exec and port-forward handoff, guarded actions, searchable log workflows, YAML/diff inspection, and a simple visual control-plane map.
