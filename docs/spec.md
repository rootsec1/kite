...
- Multi-cluster Kubernetes cockpit with kubeconfig context switching, scoped resource exploration, pinned resources, windowed sortable signal-aware inventory with inline pod and service backend diagnostics, exact Ingress/HTTPRoute-to-service topology links, pod-to-node and node-to-pod debugging jumps, pod-level status diagnostics, pod placement constraints, pod condition diagnostics, newest-first warning-prioritized pod events, exec and port-forward handoff, guarded actions, searchable log workflows, YAML/diff inspection, and a simple visual control-plane map.
